...
const ProductCard = ({ product }) => {
  ...
  const [status, setStatus] = useState('add'); // 'add' | 'update' | 'saved'
  ...
  const getButtonColor = () => {
    if (status === 'add') return '#007bff';      // синий
    if (status === 'update') return '#17a2b8'; // голубой вместо жёлтого
    if (status === 'saved') return '#28a745';    // зелёный
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ color: '#ccc', margin: '4px 0 8px', fontSize: '13px' }}>
        {product.price} ₽
      </div>
